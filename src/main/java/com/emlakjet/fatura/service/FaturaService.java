...
public class FaturaService {
     ...
     * is greater than the given value in config file, then returns an error. Otherwise
     * saves the new fatura to the database.
     *
    ...
    public Fatura addNewFatura(RequestNewFatura faturaRequest) {
        ...
        if (sumBefore.add(fatura.getAmount()).compareTo(BigDecimal.valueOf(200)) > 0)
            throw new SumOfPSFaturasExceededLimit(sumBefore, fatura.getAmount(), BigDecimal.valueOf(200));
